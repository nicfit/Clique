...
def init(env, cmd_parser):
    global DEFAULT_KEYFILE
    DEFAULT_KEYFILE = env["CLIQUE_D"] / "key"

    parser = cmd_parser.add_parser("keygen",
            help="Generate Clique (i.e. EC 256) encryption keys.")
    parser.add_argument("-f", dest="ofile", default=None, metavar="output_file",
...
def run(args):
    ...
    # TODO: Adding comments needs to be supported natively else it is too easy
    # to lose them along the way.
    '''
    cmt = args.comment or prompt("Comment (optional): ", default=None)
    if cmt:
        jwk._params["cmt"] = cmt
    ...
    if args.server:
        print("## Uploading public key to " + args.server)
        tprint = keystore().upload(jwk)
        print("## Key URL: {}/keys/{}".format(args.server, tprint))
